...
function SectionTitle({ title, right }: { title: string; right?: React.ReactNode }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 8 }}>
      <div style={{ fontSize: 13, fontWeight: 800, color: "rgba(0,0,0,0.75)" }}>{title}</div>
      {right}
    </div>
  );
}
...
function CareerProjectionDashboard({
  ...
  const ADV_METRICS: { label: string; key: keyof any }[] = [
    ...
    { label: "Transition (BU)", key: "Transition_Ball_Use" },
    { label: "Post clearance (BU)", key: "Post_Clearance_Ball_Use" },
    { label: "Clearance (BU)", key: "Clearance_Ball_Use" },
  ...
  const playerCompareRows = useMemo(() => {
    ...
    for (const [metric, rec] of byMetric.entries()) {
      ...
      flat.push({ category: rec.category, metric, a: aPct, b: bPct, diff: bPct - aPct });
    }
        ...
        <div style={{ position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              >
                {headshotUrl ? (
                  <img src={headshotUrl} alt={player?.name ?? ""} style={{ height: "100%", width: "100%", objectFit: "cover" }} />
                ) : null}
              </div>

              <div>
                <div style={{ fontSize: 14, fontWeight: 950, color: "rgba(0,0,0,0.80)" }}>Snapshot ({lastActual?.season ?? "—"})</div>
              ...
              {comparePlayer && compareHeadshotUrl ? (
                <div
                  style={{
                    height: 78,
                    width: 78,
        ...
        <Card>
          ...
          <div style={{ marginTop: 10, display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 950, color: "rgba(0,0,0,0.80)", marginBottom: 8 }}>Ball Use</div>
              {["Ball Use", "Kicking", "Handballing", "Transition (BU)", "Post clearance (BU)", "Clearance (BU)"].map((lbl) => {
                ...
                return (
                  ...
                  >
                    <div style={{ fontSize: 12, color: "rgba(0,0,0,0.78)", fontWeight: 800 }}>{r.label}</div>
            ...
            <div style={{ marginBottom: 22 }}>
              <div style={{ fontSize: 12, fontWeight: 950, color: "rgba(0,0,0,0.80)", marginBottom: 8 }}>Ball Winning</div>
            ...
            <div style={{ fontSize: 12, fontWeight: 950, color: "rgba(0,0,0,0.80)", marginTop: 2 }}>Career Outcomes</div>

            {/* AA probability */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 60px 1.3fr", gap: 10, alignItems: "center" }}>
              <div style={{ fontSize: 12, color: "rgba(0,0,0,0.78)", fontWeight: 800 }}>All Australian</div>
              <div style={{ fontSize: 12, color: "#111", fontWeight: 950, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 60px 1.3fr", gap: 10, alignItems: "center" }}>
              <div style={{ fontSize: 12, color: "rgba(0,0,0,0.78)", fontWeight: 800 }}>100+ Games</div>
              <div style={{ fontSize: 12, color: "#111", fontWeight: 950, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>
            ...
            <div style={{ marginTop: 8, fontSize: 11, color: "rgba(0,0,0,0.45)", fontStyle: "italic" }}>
              *Outcomes are based on the probabilities to reach another 100 games and earn an All Australian position
            </div>
          </div>
        </Card>
          ...
          <SectionTitle
            ...
            right={
              <span style={{ fontSize: 11, color: "rgba(0,0,0,0.55)" }}>
                Rating{comparePlayer ? " Comparison" : ""}
              </span>
            }
          />
      ...
      {comparePanelOpen && (
        <div className="comparePanel">
          <div className="comparePanelHeader">
            <div>
              <div style={{ fontSize: 13, fontWeight: 950, color: "rgba(0,0,0,0.78)", letterSpacing: -0.2 }}>Compare</div>
              ...
              <>
                <div style={{ marginTop: 14, borderTop: "1px solid rgba(0,0,0,0.08)", paddingTop: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
                    ...
                    <div style={{ fontSize: 11, color: "rgba(0,0,0,0.50)" }}>Diff = {comparePlayer.name} − {player.name}</div>
                  </div>
...
const comparisonTables = useMemo(() => {
  ...
  const scalars = [
    ...
    ...r,
    diff: r.a == null || r.b == null ? null : r.b - r.a,
  }));
...
const kpis = useMemo(() => {
  ...
  const ageRank =
    clubKpi && Number.isFinite(clubKpi.squad_age_avg)
      ? denseRank(leagueRows.map((r) => r.squad_age_avg), clubKpi.squad_age_avg, "asc")
      : null;
      ...
      {!compareTeam ? (
        ...
        <>
          <div style={{ marginTop: 14, borderTop: "1px solid rgba(0,0,0,0.08)", paddingTop: 12 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
              <div style={{ fontSize: 12, fontWeight: 950, color: "rgba(0,0,0,0.72)" }}>Head-to-head</div>
              <div style={{ fontSize: 11, color: "rgba(0,0,0,0.50)" }}>Diff = {compareClubName} − {clubName}</div>
            </div>
